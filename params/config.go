...
type ChainConfig struct {
	...
	DeliveranceBlock *big.Int `json:"deliveranceBlock,omitempty"` // Deliverance switch block (nil = no fork, 0 = already on byzantium)
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Deliverance: %v Engine: %v}",
		c.ChainId,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
func (c *ChainConfig) IsDeliverance(num *big.Int) bool {
	return isForked(c.DeliveranceBlock,num)
}
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.DeliveranceBlock, newcfg.DeliveranceBlock, head) {
		return newCompatError("Deliverance fork block", c.DeliveranceBlock, newcfg.DeliveranceBlock)
	}
